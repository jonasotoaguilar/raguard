...
- **Worker** (`apps/worker/src/raguard_worker/`): Owns CPU/IO-bound ingestion (download → `parsers.py` PDF/Markdown → `chunking.py` → `embeddings.py` via the provider-selected embedder → atomic `jobs.py` chunk insert + `cleanup.py`). Independent scaling; at-least-once via Arq lease, idempotent by document id.
