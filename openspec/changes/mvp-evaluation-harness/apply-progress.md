...
- [x] 5.1: `embedder.py` (SHA-256 token → axis `% 1536`, L2; empty text → zero vector) + `db.py`+`seed.py` (unique `raguard_eval_<hex12>` via `apps/api/alembic.ini`; `DROP … WITH (FORCE)` in `finally`; dataset topology preserved, credentials/embeddings never logged).
